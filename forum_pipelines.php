...
function forum_optimiser_base_disparus($flux){
	...
	$res = sql_select("forum.id_forum AS id",
			"spip_forum AS forum
		        LEFT JOIN spip_rubriques AS rubriques
		          ON forum.objet='rubrique' AND rubriques.id_rubrique=forum.id_objet",
			"rubriques.id_rubrique IS NULL
		         AND forum.objet='rubrique' AND forum.id_objet>0");
	...
	$res = sql_select("forum.id_forum AS id",
		        ...
		        LEFT JOIN spip_articles AS articles
		        	ON forum.objet='article' AND articles.id_article=forum.id_objet",
				"articles.id_article IS NULL
					AND forum.objet='article' AND forum.id_objet>0");
	...
	$res = sql_select("forum.id_forum AS id",
		        ...
		        LEFT JOIN spip_breves AS breves
		        	ON forum.objet='breve' AND breves.id_breve=forum.id_objet",
				"breves.id_breve IS NULL
					AND forum.objet='breve' AND forum.id_objet>0");
	...
	$res = sql_select("forum.id_forum AS id",
		        ...
		        LEFT JOIN spip_syndic AS syndic
		        	ON forum.objet='syndic' AND syndic.id_syndic=forum.id_objet",
			"syndic.id_syndic IS NULL
		         AND forum.objet='syndic' AND forum.id_objet>0");
